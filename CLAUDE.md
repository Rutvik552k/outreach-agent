...
## goal
Design and build an automation agent that increases the user's visibility in the
tech world, centered on the user's GitHub account.

- **Automated GitHub contributions to real projects**: the agent discovers
  suitable open-source repositories/issues (good-first-issue, help-wanted,
  docs gaps, failing CI, dependency bumps), prepares genuine, high-quality
  contributions (PRs, issue triage, docs fixes), and drives them through fork →
  branch → PR → review-response workflow via the GitHub API, with human
  approval gates before anything is published under the user's identity.
- **Profile growth engine**: maintain a consistent, authentic contribution
  cadence — own-repo projects, README/profile polish, pinned repos, release
  notes — so the activity graph reflects real, defensible work.
- **Strong and realistic**: every automated contribution must be substantive
  and pass maintainer review. No fake commit padding, no spam PRs, no
  contribution-graph gaming — these get accounts flagged and destroy
  reputation, the opposite of visibility. Quality over volume.
- Must respect GitHub Terms of Service, API rate limits, and per-repo
  contribution guidelines (CONTRIBUTING.md) at all times.